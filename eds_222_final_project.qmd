...
tou_reg_all_hours <- lm(formula = hourly_energy_mwh ~ tou_policy +, 
   data = energy_temp_df)
...
## Looking at the peak hours only
...
tou_peak_hours_df <- energy_temp_df |> 
  ...
  group_by(date) |> 
  summarize(peak_energy_mwh = sum(hourly_energy_mwh))

tou_policy <- energy_temp_df |> 
  select(c("date", "tou_policy", "hot_day"))

tou_peak_hours_df <- left_join(x = tou_peak_hours_df, 
                               y = tou_policy, 
                               by = "date")

tou_reg_peak_hours <- lm(formula = peak_energy_mwh ~ tou_policy, 
...
tou_reg_peak_hours <- energy_temp_df |> 
  mutate(time = as_datetime(time, format = "%H:%M:%S")) |> 
  mutate(time = lubridate::hour(time)) |> 
  filter(time >= 16 & time <= 21) |> 
  group_by(date) |> 
...
decom_ts |> 
  autoplot(daily_energy_mwh)+
  labs(title = "Time Series Plot of Daily Energy (MWh) during Peak TOU Times")

decom <- decom_ts |> 
  model(classical_decomposition(daily_energy_mwh, type = "additive")) |> 
  components() |> 
  autoplot() +
